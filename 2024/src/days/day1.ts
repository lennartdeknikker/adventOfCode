...
export async function day1(): Promise<string> {
    ...
    const firstList = parsedToNumbers.filter((_, index) => index % 2 === 0).sort((a, b) => a - b).filter(item => item);
    const secondList = parsedToNumbers.filter((_, index) => index % 2 !== 0).sort((a, b) => a - b).filter(item => item);
    ...
    return answerA + ' ' + similarityScore
}
